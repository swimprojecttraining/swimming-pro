...
# ============================================================
def cargar_fondo_interfaz(
    nombre_archivo="Fondo_de_pantalla_Swimprojecttraining.png",
):
  """Lee la imagen del fondo desde la raíz del proyecto y la inyecta mediante CSS en la app de Streamlit."""
  ruta_imagen = Path(nombre_archivo)
  ...
  if ruta_imagen.exists():
    ...
    estilos_css = f"""
        <style>
        /* Contenedor principal actualizado para versiones recientes de Streamlit */
        ...
        /* Transparencia para la cabecera superior */
        [data-testid="stHeader"] {{
            background-color: rgba(0,0,0,0);
        }}

        /* Fondo semi-transparente para el formulario para mejorar legibilidad */
        [data-testid="stForm"] {{
...
def generar_token_handshake(nombre_club, secret_key_exclusivo):
  """Genera un token firmado criptográficamente mediante HMAC-SHA256 con una validez estricta basada en tiempo Unix."""
...
# 🚀 LLAMADA A LA FUNCIÓN DE FONDO (CRÍTICO: AHORA SÍ SE EJECUTA EN CADA RERUN)
cargar_fondo_interfaz("Fondo_de_pantalla_Swimprojecttraining.png")
...
st.markdown(
    "<h3 style='text-align: center; color: gray;'>Consola Central de"
    " Acceso</h3>",
    unsafe_allow_html=True,
)
st.markdown("---")
...
try:
  ...
  resp = (
      ...
      .select("nombre_club", "url_subdominio", "club_secret_key")
      .execute()
  )
...
if conectar and dict_clubes:
  ...
  with st.spinner("Generando pase de abordaje digital seguro..."):
    ...
    st.warning(
        ...
        " Si requiere generar una nueva pantalla de acceso o cambiar de"
        " institución, simplemente **actualice esta página**."
    )
